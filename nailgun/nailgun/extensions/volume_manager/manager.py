...
from fuel_agent.drivers.nailgun import NailgunSimpleDriver
...
class VolumesHumanFormatParser(object):
    ...
    def partition_scheme(self):
        if not hasattr(self, '_partition_scheme'):
            ...
            p_scheme = PartitionScheme()
            p_scheme.lvs = NailgunSimpleDriver.parse_lv_data(
                self.parsed_dict['lvs'])
            p_scheme.pvs = NailgunSimpleDriver.parse_pv_data(
                self.parsed_dict['pvs'])
            p_scheme.fss = NailgunSimpleDriver.parse_fs_data(
                self.parsed_dict['fss'])
            p_scheme.vgs = NailgunSimpleDriver.parse_vg_data(
                self.parsed_dict['vgs'])
            p_scheme.mds = NailgunSimpleDriver.parse_md_data(
                self.parsed_dict['mds'])
            p_scheme.parteds = \
                NailgunSimpleDriver.parse_parted_data(
                    self.parsed_dict['parteds'])
