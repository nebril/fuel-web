...
class NailgunSimpleDriver(Nailgun):
    ...
    def parse_partition_scheme(self):
        ...
        partition_scheme.lvs = lvs

        raw_pvs = self.partition_data.get('pvs', {})
        pvs = self.parse_pv_data(raw_pvs)
        partition_scheme.pvs = pvs

        raw_fss = self.partition_data.get('fss', {})
        fss = self.parse_fs_data(raw_fss)
        partition_scheme.fss = fss

        raw_vgs = self.partition_data.get('vgs', {})
        vgs = self.parse_vg_data(raw_vgs)
        partition_scheme.vgs = vgs

        raw_mds = self.partition_data.get('mds', {})
        mds = self.parse_md_data(raw_mds)
        partition_scheme.mds = mds

        raw_parteds = self.partition_data.get('parteds', {})
        parteds = self.parse_parted_data(raw_parteds)
